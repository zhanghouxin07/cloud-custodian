...
class DCTest(BaseTest):
    ...
    # Action Tests
    # =========================
    def test_dc_action_tag(self):
        """Test add tag action"""
        factory = self.replay_flight_data('dc_action_tag')
        p = self.load_policy(
            {
                'name': 'dc-action-tag',
                'resource': 'huaweicloud.dc',
        ...
        resources = p.run()
        self.assertEqual(len(resources), 1)
        # Verification: Check VCR to confirm Huawei Cloud tag API was called
        # and body contains {'key': 'CostCenter', 'value': 'Finance'}
        ...
        p = self.load_policy(
            {
                ...
                'resource': 'huaweicloud.dc',
                'actions': [{'type': 'remove-tag', 'keys': ['environment']}]
            },
            session_factory=factory
        )
