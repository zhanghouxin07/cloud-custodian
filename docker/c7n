...
ARG providers="gcp azure kube openstack tencentcloud oci awscc huaweicloud"
...
ADD tools/c7n_huaweicloud/pyproject.toml tools/c7n_huaweicloud/poetry.lock /src/tools/c7n_huaweicloud/
RUN if [[ " ${providers[*]} " =~ "huaweicloud" ]]; then     . /usr/local/bin/activate &&     cd tools/c7n_huaweicloud &&     poetry install --without dev --no-root; fi
...
ADD tools/c7n_huaweicloud /src/tools/c7n_huaweicloud
RUN if [[ " ${providers[*]} " =~ "huaweicloud" ]]; then     . /usr/local/bin/activate &&     cd tools/c7n_huaweicloud &&     poetry install --only-root; fi
